...
Величина $u$ подчиняется плотности распределния вида:
...
В другом случае происходит рассеяние (*mcrt.h::PIC_scatter()*).

Выбирается новое направление из изотропного распределния $n' = (n_x', n_y',n_z')$, расчитывается $\mu = \bf n\cdot n'$. Генерируются скорости $u$ и  $w$, и на их основе вычисляется выходная частота фотона $x_o$, которая переводится в доплеровскую скорость атома $\Delta v = x_o v_{th}$.
...
$$ \phi = 2\pi\xi_1\\ \theta = arccos(2\xi_2-1)$$
тогда
$$n_x = sin(\theta) cos(\phi)\\
n_y = sin(\theta) sin(\phi)\\
n_z = cos(\theta)$$

### Выборка скоростей атома $u$ и $w$
...
$$g(u)=\begin{cases}
   \left( (x-u)^2+a^2 \right)^{-1}, u\leq u_0
   \\
   e^{-u_0^2}\left( (x-u)^2+a^2 \right)^{-1}, u>u_0
 \end{cases}$$
...
$$ x_c(a) = 1.59 - 0.6lg(a) - 0.03lg^2(a)$$
$$u_0 = 
\begin{cases}
0; 0\leq x <0.2
\\
x-0.01a^{1/6}e^{1.2x}; 0.2\leq x <x_c(a)
\\
4.5; x\geq x_c(a)
\end{cases}$$
...
здесь определение $\tau_0$ отличается от приведенного выше, $\tau_0=\sqrt\pi\sigma(x=0)N$

### Результаты расчета
Запускалось 150 000 пакетов (~7 часов) 

При наших параметрах $\tau_0 = 83438.6$ (без $\sqrt\pi$).

|Величина |Теория (Dijkstra) |Теория (Tatitsiomi) | Расчет|
|-|-|-|-|
|$x_p$|2.5 (48 км/с) |3.23 (58.7 км/с)| 3.43(62.3 км/с)|
|$t_{trap}$ |$8.3\cdot10^{-9}$ |$1.1\cdot10^{-8}$| $3.797\cdot10^{-9}$|
| $N_{scat}$|50 063 |134 432| 156 898 |
...
Запускалось 10 000 пакетов (~2 часа) 

При наших параметрах $\tau_0 = 834386$ (без $\sqrt\pi$).

|Величина |Теория (Dijkstra) |Теория (Tatitsiomi) | Расчет|
|-|-|-|-|
|$x_p$|5.4 (98.1 км/с) |6.96 (126.5 км/с)| 6.25(113.6 км/с)|
|$t_{trap}$ |$1.79\cdot10^{-7}$ |$2.32\cdot10^{-7}$| $8.92\cdot10^{-8}$|
| $N_{scat}$|500 631 |1 344 329| 930 803 |

![Профиль](imgs/prof2.png)

![Время жизни](imgs/lt2.png)

![Число рассеяний](imgs/scat2.png)
...
Фотоны, находящиеся в крыльях могут вернуться в центр линии, но для это им нужно совершить ~$x^2$ рассеяний, так как длина свободного пробега большая, в большинстве случаев они не успевают вернуться и вылетают из сферы.
